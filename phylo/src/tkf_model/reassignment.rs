...
pub struct ReassignEdge<Q: QMatrix + Display, AA: AncestralAlignment> {
    ...
    pub cost: TKF92Cost<Q, AA>,
}

impl<Q: QMatrix + Display, AA: AncestralAlignment> ReassignEdge<Q, AA> {
    pub fn new(cost: TKF92Cost<Q, AA>) -> ReassignEdge<Q, AA> {
    ...
    pub fn fill_dp(&mut self, v2_idx: &NodeIdx) {
        ...
        // TODO: i dont really need this anymore if the virtual rerooting is done
        ...
        for (i, assignment) in self.backtracking().0.iter().enumerate() {
            let original = vec![
                self.cost.phylo.msa.ancestral_map(&v1_idx)
                    ...
                    .is_some(),
                self.cost.phylo.msa.ancestral_map(v2_idx)
                    [self.cost.model_info.borrow().blocks[i] - 1]
                    .is_some(),
            ];
    ...
    // The actions are for a specific assignment of the two re-estimated nodes.
    fn get_possible_events_for_assignment(
        ...
        for (typ, (edge, edge_id)) in dirty_edges {
            match actions.get(typ).unwrap() {
                ...
                Action::Nothing => {
                    // previous event cant be a deletion
                    if edge == &self.cost.phylo.tree.root || block_id == 0 {
                        choices[*edge_id] = vec![false];
                    } else {
                        choices[*edge_id] = vec![false, true];
                    }
                }
                // Insertion and Homologous (i.e. no Deletion)
        ...
        for (typ, (node, _)) in dirty_edges {
            ...
            if node == &self.cost.phylo.tree.root {
                ...
                x *= match actions.get(typ).unwrap() {
                    ...
                    Action::Deletion => n0(m, b),
                    Action::Homolog => h1(l, m, b, time),
                    _ => 1.0,
                }
            }
        }
    ...
    fn bools_to_index(bools1: &[bool; 2], bools2: &[bool]) -> usize {
        bools1
            .iter()
            .chain(bools2.iter())
    ...
    fn get_dirty_edges(&self, v2_idx: &NodeIdx) -> HashMap<DirtyTreeEdge, (NodeIdx, usize)> {
        ...
        edges_to_remove.insert(DirtyTreeEdge::V2, (*v2_idx, 0));
        let t3_and_t4 = &old_tree.node(v2_idx).children;
        edges_to_remove.insert(DirtyTreeEdge::T3, (t3_and_t4[0], 1));
        edges_to_remove.insert(DirtyTreeEdge::T4, (t3_and_t4[1], 2));
        let v1_idx = &old_tree.node(v2_idx).parent.unwrap();
        edges_to_remove.insert(DirtyTreeEdge::V1, (*v1_idx, 3));
        let sibling = &old_tree.sibling(v2_idx).unwrap();
        edges_to_remove.insert(DirtyTreeEdge::T2, (*sibling, 4));
        edges_to_remove
    }
